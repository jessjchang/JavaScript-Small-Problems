...
PROBLEM:
- implement a stack-and-register-based programming language
- Input: string
- Output: log to console if need to PRINT

Rules:
- stack should be initialized to []
- register should be initialized to 0
- operations in string argument will be separated by space
- don't have to worry about invalid arguments
- all operations are integer operations, i.e. return an integer and not a floating point number
...
ALGORITHM:
- takes string `program` as argument
- initialize `stack` to []
- initialize `register` to 0
- split the string argument `program` to array of separate commands -`commandArray`
- iterate through `commandArray` and on each iteration for each command, perform the necessary action
*/

function minilang(program) {
  let stack = [];
  let register = 0;
  let commandArray = program.split(' ');
